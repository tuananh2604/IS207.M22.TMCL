...
class Products
{
	...
	public function addProduct($product_name,
		...
		if($product_rate>5 || $product_rate<0){
			return ['status'=> 303, 'message'=> 'Rating must in range 0 to 5'];
		}
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				$uniqueImageName = time()."_".$this->convert_name($product_name).".".$ext;
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/ecommerce/product_images/".$uniqueImageName)) {
					
					$q = $this->con->query("INSERT INTO `products`(`product_cat`, `product_brand`, `product_title`, `product_qty`, `product_price`, `product_desc`, `product_image`, `product_keywords`,`product_oriprice`,`product_rate`) VALUES ('$category_id', '$brand_id', '$product_name', '$product_qty', '$product_price', '$product_desc', '$uniqueImageName', '$product_keywords','$product_oriprice','$product_rate')");

					if ($q) {
						return ['status'=> 202, 'message'=> 'Product Added Successfully..!'];
						
					}else{
						return ['status'=> 303, 'message'=> $this->con->connect_error];
					}
	...
	// public function getPrimaryImage($pid){
	// 	$q = $this->con->query("SELECT product_image FROM products WHERE product_id="+$pid);
	// 	if ($q->num_rows > 0) {
	// 		$row = $q->fetch_assoc() ;
	// 		$primaryImg = $row['product_image'];
	// 		return $primaryImg;
	// 	}
	// }
	public function getExtraImages($pid){
		$q = $this->con->query("SELECT * FROM images WHERE p_id='$pid'");
		$extraImgs = [];
		if ($q->num_rows > 0) {
			while ($row = $q->fetch_assoc()) {
				$extraImgs[] =$row ;
		...
		return ['status'=> 202, 'message'=> $extraImgs];
	}

	public function editPrimaryImg($pid,$file){
		$fileName = $file['name'];
		$fileNameAr= explode(".", $fileName);
		$extension = end($fileNameAr);
		$ext = strtolower($extension);
		$q = $this->con->query("SELECT * FROM products WHERE product_id=$pid");
		$row = $q->fetch_assoc();
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png"){
			if ($file['size'] > (1024 * 2)) {
				$uniqueImageName = time()."_".$this->convert_name($product_name).".".$ext;
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/ecommerce/product_images/".$uniqueImageName)){
					$q = $this->con->query("UPDATE `products` SET `product_image` = '$uniqueImageName' WHERE product_id = '$pid'");
					if ($q) {
						return ['status'=> 202, 'message'=> 'Update Primary Image Successfully...!'];
					}else{
	...
	public function addExtraImg($pid,$file){
		$q = $this->con->query("SELECT * FROM products WHERE product_id=$pid");
		$row = $q->fetch_assoc();
		$product_name = $row["product_title"];
		foreach($file["tmp_name"] as $key=>$tmp_name) {
			...
			if ($ext == "jpg" || $ext == "jpeg" || $ext == "png"){
				if ($file['size'][$key] > (1024 * 2)) {
					$uniqueImageName = time()."_".$key."_".$this->convert_name($product_name).".".$ext;
					if (move_uploaded_file($file['tmp_name'][$key], $_SERVER['DOCUMENT_ROOT']."/ecommerce/product_images/".$uniqueImageName)){
						$q = $this->con->query("INSERT INTO `images` (`p_id`, `img_name`) VALUES ('$pid','$uniqueImageName')");
						if (!$q) {
							return ['status'=> 303, 'message'=> 'Failed to run query'];
						}
					}else{
		...
		return ['status'=> 202, 'message'=> 'Update Extra Images Successfully...!'];
	}

	public function deleteExtraImg($pid=null,$removeImgs){
		if($pid){
			for($i =0 ; $i<count($removeImgs);$i++){
				$img_id = $removeImgs[$i];
				$q = $this->con->query("DELETE FROM `images` Where img_id = '$img_id'");
				if (!$q) {
					return ['status'=> 303, 'message'=> 'Failed to run query'];
				}
			}
			return ['status'=> 202, 'message'=> 'Images removed'];
		}
	}
	...
	public function editProductWithImage($pid,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				$uniqueImageName = time()."_".$product_name.".".$ext;
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/ecommerce/product_images/".$uniqueImageName)) {
...
if (isset($_POST['edit_product'])) {
	...
	&& !empty($e_product_rate)
	&& !empty($e_product_qty)
	&& !empty($e_product_oriprice)
...
if (isset($_POST['edit_image'])){
	...
	if (!empty($pid)){
		$p = new Products();
		$result_PrimaryImg=["status"=>202,"message"=>""];
		$result_ExtraImg=["status"=>202,"message"=>""];;
		$result_RemoveImg=["status"=>202,"message"=>""];;
		if (isset($_FILES['primaryImage']['name']) && !empty($_FILES['primaryImage']['name'])) {
			$result_PrimaryImg = $p->editPrimaryImg($pid,$_FILES['primaryImage']);
		}
		if (isset($_FILES['extraImages']['name']) && !empty($_FILES['extraImages']['name'][0])) {
			$result_ExtraImg = $p->addExtraImg($pid,$_FILES['extraImages']);
		}
		if(isset($del_extra_img)){
			$result_RemoveImg = $p->deleteExtraImg($pid,$del_extra_img);
		}
		echo json_encode(['primaryImg'=>$result_PrimaryImg, 'extraImg'=>$result_ExtraImg, 'deleteImg'=>$result_RemoveImg]);
		exit();
...
if(isset($_POST['getProductImage'])){
	$p = new Products();
	echo json_encode($p->getExtraImages($_POST['pid']));
	exit();
}
